...
public class MemoryCardMappableDAO extends AbstractMappableDAO<MemoryCardEntity, MemoryCardModel> {
    ...
    public void deleteDataOnRecto(long idCard,long idData){
        Optional<MemoryCardEntity>memoryCardEntity =this.getRepository().findById(idCard);
        if(memoryCardEntity.isPresent()){
            memoryCardEntity.get().setRecto(
                memoryCardEntity.get().getRecto().stream().filter(
                        x->x.getId()!=idData
    ...
    public void deleteDataOnVerso(long idCard,long idData){
        Optional<MemoryCardEntity>memoryCardEntity =this.getRepository().findById(idCard);
        if(memoryCardEntity.isPresent()){
            memoryCardEntity.get().setVerso(
                    memoryCardEntity.get().getVerso().stream().filter(
                            x->x.getId()!=idData
                    ).collect(Collectors.toList())
            );
            this.getRepository().save(memoryCardEntity.get());
